...
def carregar_blocos():
    """Carrega os blocos do índice armazenado na sessão."""

    docs = list(st.session_state["index"].docstore.docs.values())
    return docs
    ...
    for x in temas:
        for y in x.split(','):
            if y not in temas_unicos and y!='':
                ...
                temas_do_bloco = y.strip().split(',')
                for tema_individual in temas_do_bloco:
                    lista_de_temas.append(tema_individual)
    ...
    with st.expander("🔍 Ver temas detectados por bloco"):
        if modo != 'cloud':
            for i, doc in enumerate(documentos):
                st.markdown(f"**Bloco {i+1}** - Temas: `{doc.metadata.get('tema', 'outros')}`")
                st.caption(doc.text[:200] + "...")  # Mostra trecho inicial
        
        elif modo == 'cloud':
            for i, doc in enumerate(documentos):
                st.markdown(f"**Bloco {i+1}** - Temas: `{doc.metadata.get('temas', 'outros')}`")
                st.caption(doc.text[:200] + "...")  # Mostra trecho inicial
